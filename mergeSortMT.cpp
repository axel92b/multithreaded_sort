...
void mergeSortAuxMT(int* arr, int* temp, int size, int cur_depth, int max_depth){
  ...
  if (cur_depth <= max_depth) {
    std::thread t(mergeSortAuxMT2, arr, temp, left, cur_depth+1, max_depth);
    std::thread t1(mergeSortAuxMT2, (arr + left), temp, (size - left), cur_depth+1, max_depth);
    t.join();
    t1.join();
  } else {
...
void mergeSortMT(int arr[], int size, int max_depth){
  ...
  int depth = 1;
  mergeSortAuxMT2(arr, temp, size, depth, max_depth);
  delete[] temp;
}